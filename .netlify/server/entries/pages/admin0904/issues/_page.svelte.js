...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  return `${validate_component(Navbar, "Navbar").$$render($$result, {}, {}, {})} <main class="container mx-auto"><h1 class="text-3xl font-bold mt-20 mx-3" data-svelte-h="svelte-1xr6epi">🕹 이슈 관리</h1> <div class="text-right m-3" data-svelte-h="svelte-69e8ya"><a href="issues/register" class="btn btn-primary">이슈 등록</a></div> <div class="overflow-x-auto m-3"><table class="table table-zebra text-center"><thead data-svelte-h="svelte-18i829"><tr> <th>분류</th> <th>제목</th> <th>마감일</th> <th>참여인원</th> <th>생성일</th> <th>상태</th> <th>공지</th> <th>수정</th></tr></thead> <tbody>${each(data.issues, (issue) => {
    return `<tr> <td>${escape(issue.category)}</td> <td><a${add_attribute("href", issue.link, 0)} target="_blank" class="link link-primary">${escape(issue.title)}</a></td> <td>${escape(formatDate(issue.dueDate))}</td> <td>${escape(issue.participants.length.toLocaleString())}</td> <td>${escape(formatDate(issue.createdAt))}</td> <td>${escape(issue.isPublic)}</td> <td>${escape(issue.isNotice ? "🔥" : "")}</td> <td><a href="${"issues/" + escape(issue._id, true)}" class="btn btn-primary btn-xs">수정</a></td> </tr>`;
